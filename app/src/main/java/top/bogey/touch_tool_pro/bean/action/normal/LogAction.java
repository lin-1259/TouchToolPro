...
public class LogAction extends NormalAction {
    ...
    public void execute(TaskRunnable runnable, FunctionContext context, Pin pin) {
        ...
        if (log.getValue() != null && !log.getValue().isEmpty()) {
            PinBoolean save = (PinBoolean) getPinValue(runnable, context, savePin);
            if (save.isBool()) {
                SaveRepository.getInstance().addLog(runnable.getTask().getId(), runnable.getStartAction().getFullDescription() + ":" + log.getValue());
            }
            PinBoolean toast = (PinBoolean) getPinValue(runnable, context, toastPin);
            if (toast.isBool()) {
                MainAccessibilityService service = MainApplication.getInstance().getService();
                service.showToast(log.getValue());
            }
        }
